...
const EditProfile = () => {
  ...
  const { getUserDataById, updateUserProfileOnly } = useUserAuth();
  ...
  const handleSave = async (event) => {
    ...
    setIsLoading(true);
    const { success, error } = await updateUserProfileOnly(id, formData);
    setIsLoading(false);
    if (success) {
      sendActionToBot("profile_edit_successful");
      closeWebApp();
    } else {
  ...
  return (
    <Box sx={styles.container}>
      ...
      <Typography sx={{ margin: "0 10px 10px 10px" }}>
        Please edit your specifications. Your data will be kept annonymus and
